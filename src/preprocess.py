...
def main():
    print(f"[INFO] Reading data from '{config.RAW_REVIEWS_FILE}'...")
    try:
    ...
    print("[INFO] Detecting languages and cleaning text...")
    ...
    print(f"[INFO] Reseñas encontradas: {len(df)}")
    print(f"[INFO] Reseñas útiles (ES/EN): {len(df_filtered)}")
    print(f"   - Español: {len(df_filtered[df_filtered['language']=='es'])}")
    print(f"   - Inglés:  {len(df_filtered[df_filtered['language']=='en'])}")

    print(f"[INFO] Saving to '{config.PROCESSED_REVIEWS_FILE}'...")
